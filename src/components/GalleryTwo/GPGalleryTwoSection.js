import React, {Component} from 'react';
import Link from "next/link";

class GpGalleryTwoSection extends Component {
	render() {
		return (
			<div className="gallery-area pt-120 pb-60 gray-bg mb-120">
				<div className="container">
					<div className="row">
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/1.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/1.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Fresh Orange</a></Link></h3>
									<span>Organic Fruits</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/2.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/2.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Fresh Strawberry</a></Link></h3>
									<span>Organic Fruits</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/3.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/3.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Orange Tomato</a></Link></h3>
									<span>Vegetables</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/4.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/4.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Fresh Milks &amp; Meat</a></Link></h3>
									<span>Organic Fruits</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/5.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/5.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Organic Carrots</a></Link></h3>
									<span>Fresh Food</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/6.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/6.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Orange Seeds</a></Link></h3>
									<span>Vegetables</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/7.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/7.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Fresh Orange</a></Link></h3>
									<span>Organic Fruits</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/8.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/8.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Fresh Milks &amp; Meat</a></Link></h3>
									<span>Organic Fruits</span>
								</div>
							</div>
						</div>
						<div className="col-xl-4 col-lg-4 col-md-6">
							<div className="gallery-wrapper mb-60">
								<div className="gallery-img">
									<Link href="#" as="#" >
										<a>
											<img src={require('../../../public/assets/img/gallery/gallery2/9.jpg')} alt='image'/>
										</a>
									</Link>
									<div className="gallery-icon">
										<a className="popup-image"
										   href={require('../../../public/assets/img/gallery/gallery2/9.jpg')}>
											<i className="ti-plus"></i>
										</a>
									</div>
								</div>
								<div className="gallery-text text-center">
									<h3><Link href="#" as="#" ><a >Orange Tomato</a></Link></h3>
									<span>Organic Fruits</span>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		);
	}
}

export default GpGalleryTwoSection;